...
| **高级** | 构建自定义工具、创建技能、生成训练数据、参与项目贡献 | [架构](/developer-guide/architecture) → [添加工具](/developer-guide/adding-tools) → [创建技能](/developer-guide/creating-skills) → [批处理](/user-guide/features/batch-processing) → [贡献指南](/developer-guide/contributing) | 约 4–6 小时 |
...
| **训练数据生成** | 生成用于训练或评估的 agent 轨迹 | [批处理](/user-guide/features/batch-processing) |
